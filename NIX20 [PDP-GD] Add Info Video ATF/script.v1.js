(() => {
  'use strict';
  const testInfo = {
    className: 'nix-20-test',
    debug: 0,
    testName: 'NIX20: [PDP-GD] Add Info Video ATF',
    testVersion: '0.0.1'
  };
  ...
  const loadTest = () => {
    ...
    waitForElement('.vc_row.wpb_row.vc_row-fluid.vc_row-has-fill .wpb_column.vc_column_container').then(() => {
      const video_html = `<div class="video_container text">
        ...
        <p class="video_anchor">Learn more about the Nix Mini 3</p>
      </div>
      <div class="popup-overlay" id="videoPopup">
        <div class="popup-content">
          ...
          <iframe src="https://player.vimeo.com/video/791289173?h=79772d4eac&badge=0&autopause=0&player_id=0&app_id=58479" target="_blank" title="description" allow="autoplay; fullscreen"></iframe>
        </div>
      </div>`;
      ...
      document.querySelector(".section-3 .wpb_column .vc_column-inner").insertAdjacentHTML('afterbegin', video_html);
      
      // Move popup to the end of the body to ensure it has the highest stacking context
      document.body.appendChild(document.getElementById('videoPopup'));
      ...
      // Hide popup if click close button
